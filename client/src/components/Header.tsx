    ...
    <header className="sticky top-0 z-50 bg-card border-b border-border" role="banner">
      <div className="flex items-center justify-between p-4">
        ...
        <div className="flex items-center space-x-3">
          <Button
            size="icon"
            className="relative w-12 h-12 rounded-full bg-gradient-to-r from-orange-500 to-red-500 hover:from-orange-600 hover:to-red-600 focus:ring-2 focus:ring-primary focus:ring-offset-2"
            onClick={onCartClick}
            aria-label={`Shopping cart with ${itemCount} ${itemCount === 1 ? 'item' : 'items'}. Click to view cart.`}
            aria-describedby={itemCount > 0 ? "cart-count-description" : undefined}
          ...
          >
            ...
            {itemCount > 0 && (
              <>
                ...
                <span id="cart-count-description" className="sr-only">
                  Cart contains {itemCount} {itemCount === 1 ? 'item' : 'items'}
                </span>
              </>
            )}
          </Button>
          ...
          <Sheet open={isMenuOpen} onOpenChange={setIsMenuOpen}>
            <SheetTrigger asChild>
              <Button
                size="icon"
                variant="ghost"
                className="w-12 h-12 focus:ring-2 focus:ring-primary focus:ring-offset-2"
            ...
            >
              <div className="space-y-6">
                <h2 id="navigation-menu-title" className="font-heading text-2xl font-bold">Navigation Menu</h2>
                
                <nav className="space-y-4" role="navigation" aria-label="Menu options">
                  {/* Admin Panel Link - Only visible to administrators */}
                  {isAdmin && (
                    <Button
                      variant="ghost"
                      className="w-full justify-start text-left h-auto p-4 border-2 border-red-200 bg-red-50 hover:bg-red-100 dark:border-red-800 dark:bg-red-950/20 dark:hover:bg-red-950/40 focus:ring-2 focus:ring-red-600 focus:ring-offset-2"
                      onClick={() => setIsMenuOpen(false)}
                      data-testid="menu-button-admin-panel"
                      aria-label="Go to Admin Panel - administrative functions"
                      asChild
                    >
                      <Link href="/admin">
                        <div className="flex items-center space-x-3">
                  ...
                  <Button
                    variant="ghost"
                    className="w-full justify-start text-left h-auto p-4 focus:ring-2 focus:ring-primary focus:ring-offset-2"
                    onClick={() => handleMenuAction(onShowAbout, 'about')}
                    data-testid="menu-button-about"
                    aria-label="View information about Concessions Connection"
                  >
                    <div className="flex items-center space-x-3">
                  ...
                  <Button
                    ...
                    aria-label="Share Concessions Connection with others"
                  >
                    <div className="flex items-center space-x-3">
                      <i className="fas fa-share-alt text-lg" aria-hidden="true"></i>
                  ...
                  <Button
                    ...
                    aria-label="View QR code for easy access to our menu"
                  >
                    <div className="flex items-center space-x-3">
                      <i className="fas fa-qrcode text-lg" aria-hidden="true"></i>
                  ...
                  <Button
                    ...
                    aria-label="View mobile app information and download options"
                  >
                    <div className="flex items-center space-x-3">
                      <i className="fas fa-mobile-alt text-lg" aria-hidden="true"></i>
                  ...
                  {user && pointsStatus?.pointsEnabled && (
                    <Button
                      variant="ghost"
                      className="w-full justify-start text-left h-auto p-4 bg-yellow-50 hover:bg-yellow-100 dark:bg-yellow-950/20 dark:hover:bg-yellow-950/40 border border-yellow-200 dark:border-yellow-800 focus:ring-2 focus:ring-yellow-600 focus:ring-offset-2"
                      onClick={() => {
                        setShowPointsSettings(true);
                        setIsMenuOpen(false);
                      }}
                      data-testid="menu-button-points"
                      aria-label={`View loyalty points. You have ${pointsStatus.totalPoints} points.`}
                    >
                      <div className="flex items-center justify-between w-full">
                        ...
                        <Badge 
                          ...
                          aria-label={`${pointsStatus.totalPoints} points earned`}
                        >
                  ...
                  {user && (
                    <Button
                      ...
                      aria-label={pointsStatus?.pointsEnabled ? 'Manage loyalty points settings' : 'Enable loyalty points program'}
                    >
                      <div className="flex items-center space-x-3">
                  ...
                  <div className="pt-4 border-t border-border" role="region" aria-label="User account information">
                    {user ? (
                      <>
                        <div className="px-4 py-2 mb-4 bg-muted rounded-lg" role="region" aria-label="Current user information">
                          <div className="flex justify-between items-start">
                            <div>
                              <p className="text-sm font-medium" aria-label={`Logged in as ${user.email}`}>{user.email}</p>
                              <p className="text-xs text-muted-foreground" aria-label={`Account type: ${isAdmin ? 'Administrator' : 'Customer'}`}>
                                {isAdmin ? 'Administrator' : 'Customer'}
                              </p>
                            </div>
                            {pointsStatus?.pointsEnabled && (
                              <div className="text-right" aria-label="Loyalty points summary">
                                <div className="flex items-center space-x-1">
                                  <Star className="w-3 h-3 text-yellow-600" aria-hidden="true" />
                        ...
                        <Button
                          ...
                          aria-label={`Logout from account ${user.email}`}
                        >
                          <div className="flex items-center space-x-3">
                      ...
                      <Button
                        ...
                        aria-label="Go to login page to sign in to your account"
                        asChild
                      >
                        <Link href="/login">
                          <div className="flex items-center space-x-3">
